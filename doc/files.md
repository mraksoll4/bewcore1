...
The data directory is the default location where the BewCore files are stored.
...
---------|--------------------
Linux    | `$HOME/.bewcore/`
macOS    | `$HOME/Library/Application Support/Bewcore/`
Windows  | `%APPDATA%\Bewcore\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `bewcore.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Bewcore blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `bewcore.conf`        | User-defined [configuration settings](bewcore-conf.md) for `bewcored` or `bewcore-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `bewcored.pid`        | Stores the process ID (PID) of `bewcored` or `bewcore-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `bewcored` or `bewcore-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [bewcore.conf](bewcore-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`bewcore-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by BewCore:

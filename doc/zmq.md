...
The BewCore daemon can be configured to act as a trusted "border
router", implementing the bewcore wire protocol and relay, making
...
The ZeroMQ feature in BewCore requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bewcored:
...
terminator). These options can also be provided in bewcore.conf.
...
From the perspective of bewcored, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bewcored directly. Furthermore, no information is
...
using. Bewcored appends an up-counting sequence number to each

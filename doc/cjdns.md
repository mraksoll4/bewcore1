# CJDNS support in BewCore

It is possible to run BewCore over CJDNS, an encrypted IPv6 network that
...
done outside of BewCore, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bewcore network and individual nodes.
...
before it will work with your BewCore node. This is described in steps
...
## Run BewCore with CJDNS

Once you are connected to the CJDNS network, the following BewCore
...
When enabled, this option tells BewCore that it is running in an
...
IPv6 local network. This helps BewCore perform better address management:
...
CJDNS support was added to BewCore in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bewcore-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/mraksoll4/bewcore/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BewCore

There are several ways to see your CJDNS address in BewCore:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bewcore-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bewcore-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bewcore-cli help getnodeaddresses` for details.

# User-space, Statically Defined Tracing (USDT) for BewCore

BewCore includes statically defined tracepoints to allow for more
...
**Note**: BewCore uses temporary clones of the _main_ UTXO cache
...
Multiple tools can list the available tracepoints in a `bewcored` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BewCore, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/bewcored
stap net        outbound_message 0x0000000000107c05 /src/bewcored
stap validation block_connected  0x00000000002fb10c /src/bewcored
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BewCore.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bewcored | grep NT_STAPSDT -A 4 -B 2

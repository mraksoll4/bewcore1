...
Below are some notes on how to build BewCore for Windows.

The options known to work for building BewCore on Windows are:
...
Note that for WSL the BewCore source path MUST be somewhere in the default mount file system, for
example /usr/src/bewcore, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bewcore`, for example:

    make install DESTDIR=/mnt/c/workspace/bewcore

...
Currently, it is possible to create a multisig wallet using BewCore only.

Although there is already a brief explanation about the multisig in the [Descriptors documentation](https://github.com/mraksoll4/bewcore/blob/master/doc/descriptors.md#multisig), this tutorial proposes to use the signet (instead of regtest), bringing the reader closer to a real environment and explaining some functions in more detail.
...
Before starting this tutorial, start the bewcore node on the signet network.

```bash
./src/bewcored -signet -daemon
```
...
At the time of writing, there is no way to extract a specific path from wallets in BewCore. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./src/bewcore-cli -signet createwallet "participant_${n}"
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, BewCore single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in BewCore will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./src/bewcore-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./src/bewcore-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./src/bewcore-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./src/bewcore-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
Documentation for these and other parameters can be found by typing `./src/bewcore-cli help importdescriptors`.
...
```bash
./src/bewcore-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./src/bewcore-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./src/bewcore-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./src/bewcore-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./src/bewcore-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./src/bewcore-cli -a $receiving_address
```
...
```bash
./src/bewcore-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSBT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed Bewcore Transaction (PSBT).

PSBT is a data format that allows wallets and other tools to exchange information about a Bewcore transaction and the signatures necessary to complete it. [[source](https://bitcoinops.org/en/topics/psbt/)]

The current PSBT version (v0) is defined in [BIP 174](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid bewcore address.
...
```bash
balance=$(./src/bewcore-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./src/bewcore-cli -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./src/bewcore-cli -signet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./src/bewcore-cli -signet decodepsbt $funded_psbt

./src/bewcore-cli -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./src/bewcore-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./src/bewcore-cli -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./src/bewcore-cli -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./src/bewcore-cli -signet finalizepsbt $combined_psbt | jq -r '.hex')

./src/bewcore-cli -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./src/bewcore-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./src/bewcore-cli -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./src/bewcore-cli -signet finalizepsbt $psbt_2 | jq -r '.hex')

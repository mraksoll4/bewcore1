...
{
    // Check POW's
    bool powResult1 = fCheckPOW ? CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams) : true;
    bool powResult2 = fCheckPOW ? CheckProofOfWork(block.GetPoWHash2(), block.nBits, consensusParams) : true;
...
bool HasValidProofOfWork(const std::vector<CBlockHeader>& headers, const Consensus::Params& consensusParams)
{
    return std::all_of(headers.cbegin(), headers.cend(),
            [&](const auto& header) { 
                bool check1 = CheckProofOfWork(header.GetPoWHash(), header.nBits, consensusParams);
                bool check2 = CheckProofOfWork(header.GetPoWHash2(), header.nBits, consensusParams);

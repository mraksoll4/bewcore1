    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart bewcored with -walletcrosschain to override.");
                return false;
            }
        }
    }
...
int CWallet::GetTxBlocksToMaturity(const CWalletTx& wtx) const
{
    ...
    if (auto* conf = wtx.state<TxStateConfirmed>()) {
        int block_number = conf->confirmed_block_height;
        if (block_number < 200) {
            return std::max(0, (COINBASE_MATURITY2+1) - chain_depth);
        } else {
            return std::max(0, (COINBASE_MATURITY+1) - chain_depth);
        }
    }

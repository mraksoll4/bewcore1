This directory contains the source code for the BewCore graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Bewcore UI.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bewcore:... link from a browser).
...
* `bitcoinunits.(h/cpp)`: BEW / mBEW / etc. handling
* `callback.h`
...
4. Enter `bewcore-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bewcore-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bewcore-qt`
9. You're all set! Start developing, building, and debugging the BewCore GUI

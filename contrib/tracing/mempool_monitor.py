...
""" Example logging Bewcore mempool events using the mempool:added,
...
def main(bewcored_path):
    bewcored_with_usdts = USDT(path=str(bewcored_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bewcored_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bewcored_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bewcored_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bewcored_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bewcored_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/bewcored")
        exit(1)

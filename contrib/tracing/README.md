...
USDT support in BewCore see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `bewcored` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `bewcored` binary. The paths in the examples can be changed, for
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/bewcored
```
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/bewcored
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/bewcored
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/bewcored
```

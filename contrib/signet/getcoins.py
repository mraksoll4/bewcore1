...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bewcore-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bewcore-cli', help='bewcore-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bewcore address to which the faucet should send')
...
parser.add_argument('bewcore_cli_args', nargs='*', help='Arguments to pass on to bewcore-cli (default: -signet)')

args = parser.parse_args()

if args.bewcore_cli_args == []:
    args.bewcore_cli_args = ['-signet']


def bewcore_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bewcore_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bewcore_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bewcore_cli(['getnewaddress', 'faucet', 'bech32'])

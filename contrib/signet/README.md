...
Syntax: `getcoins.py [-h|--help] [-c|--cmd=<bewcore-cli path>] [-f|--faucet=<faucet URL>] [-a|--addr=<signet bech32 address>] [-p|--password=<faucet password>] [--] [<bewcore-cli args>]`

* `--cmd` lets you customize the bewcore-cli path. By default it will look for it in the PATH
...
* `--addr` lets you specify a Signet address; by default, the address must be a bech32 address. This and `--cmd` above complement each other (i.e. you do not need `bewcore-cli` if you use `--addr`)
...
To mine the first block in your custom chain, you can run:
    ...
    CLI="./bewcore-cli -conf=mysignet.conf"

...
## 2. Importing `TestShell` from the BewCore repository

We can import the `TestShell` by adding the path of the BewCore
...
| `noshutdown` | `False` | Does not stop bewcored instances after `shutdown` if set to `True`. |

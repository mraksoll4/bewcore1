...
class MiningTest(BitcoinTestFramework):
    ...
    def test_blockmintxfee_parameter(self):
        ...
        for blockmintxfee_sat_kvb in (DEFAULT_BLOCK_MIN_TX_FEE, 0, 50, 100, 500, 2500, 5000, 21000, 333333, 2500000):
            blockmintxfee_bew_kvb = blockmintxfee_sat_kvb / Decimal(COIN)
            if blockmintxfee_sat_kvb == DEFAULT_BLOCK_MIN_TX_FEE:
                ...
                blockmintxfee_parameter = f"-blockmintxfee={blockmintxfee_bew_kvb:.8f}"
            ...
            tx_with_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=blockmintxfee_bew_kvb)
            assert_equal(tx_with_min_feerate["fee"], get_fee(tx_with_min_feerate["tx"].get_vsize(), blockmintxfee_bew_kvb))
            if blockmintxfee_bew_kvb > 0:
                lowerfee_bew_kvb = blockmintxfee_bew_kvb - Decimal(10)/COIN  # 0.01 sat/vbyte lower
                tx_below_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=lowerfee_bew_kvb)
                assert_equal(tx_below_min_feerate["fee"], get_fee(tx_below_min_feerate["tx"].get_vsize(), lowerfee_bew_kvb))
            else:  # go below zero fee by using modified fees
                tx_below_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=blockmintxfee_bew_kvb)
    ...
    def run_test(self):
        ...
        peer.wait_for_getheaders(timeout=100000)  # Drop the first getheaders

...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 BEW) in its wallet. Using them is much more
...
with the bewcored(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a bewcored.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the BewCore node application logic. For custom behaviour, subclass the
...
Test objects for interacting with a bewcored node over the p2p interface.

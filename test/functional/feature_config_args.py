...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in bewcore.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "bewcore_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'bewcore.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "bewcore_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # (conf_path, not node.bewcoreconf)
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a bewcore.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bewcore.conf" file which is ignored, because a '
        ...
        # the same bewcore.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/bewcore.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when bewcore.conf in the default data directory sets another datadir '
                      'and it contains a different bewcore.conf file that would be ignored')
        ...
        # bewcore.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "bewcore.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bewcore.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/bewcore.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "bewcore.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "bewcore.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_bewcore.conf"

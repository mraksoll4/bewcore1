...
"""Test objects for interacting with a bewcored node over the p2p protocol.

The P2PInterface objects interact with the bewcored nodes under test using the
...
class P2PConnection(asyncio.Protocol):
    ...
    def peer_connect(self, dstaddr, dstport, *, net, timeout_factor):
        ...
        logger.debug('Connecting to Bewcore Node: %s:%d' % (self.dstaddr, self.dstport))
    ...
    def peer_accept_connection(self, connect_id, connect_cb=lambda: None, *, net, timeout_factor):
        ...
        logger.debug('Listening for Bewcore Node with id: {}'.format(connect_id))
...
class P2PInterface(P2PConnection):
    """A high-level P2P interface class for communicating with a Bewcore node.
    ...
    def wait_for_getheaders(self, timeout=600):
...
class P2PDataStore(P2PInterface):
    ...
    def send_blocks_and_test(self, blocks, node, *, success=True, force_send=False, reject_reason=None, expect_disconnect=False, timeout=300):
...
class P2PTxInvStore(P2PInterface):
    ...
    def wait_for_broadcast(self, txns, timeout=300):

...
TMPDIR_PREFIX = "bewcore_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    """Base class for a bewcore test script.

    Individual bewcore test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def __init__(self) -> None:
        ...
        self.rpc_timeout = 300  # Wait for up to 60 seconds for the RPC server to respond
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bewcoreds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop bewcoreds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bewcore-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "bewcored": ("bewcored", "BEWCORED"),
            "bewcore-cli": ("bewcorecli", "BEWCORECLI"),
            "bewcore-util": ("bewcoreutil", "BEWCOREUTIL"),
            "bewcore-wallet": ("bewcorewallet", "BEWCOREWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bewcoreds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("BewcoreRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bewcored', self.options.bewcored) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bewcore-cli', self.options.bewcorecli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bewcored test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bewcored test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bewcored's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bewcored=self.options.bewcored,
                    bewcore_cli=self.options.bewcorecli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bewcore.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if bewcored has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bewcored has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if bewcored has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bewcored has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bewcore-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bewcore-wallet has not been compiled")

    def skip_if_no_bitcoin_util(self):
        """Skip the running test if bewcore-util has not been compiled."""
        if not self.is_bitcoin_util_compiled():
            raise SkipTest("bewcore-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bewcore-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bewcore-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bewcore-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bewcore-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoin_util_compiled(self):
        """Checks whether bewcore-util was compiled."""

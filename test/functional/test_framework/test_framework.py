...
class BitcoinTestMetaClass(type):
    ...
    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BewcoreTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BewcoreTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BewcoreTestFramework subclasses may not override "
                                "'__init__' or 'main'")
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bewcored', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bewcore-cli', self.options.bitcoincli) for v in versions]

...
"""Class for bewcored node under test"""
...
class TestNode():
    """A class for representing a bewcored node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, bewcored, bewcore_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        """
        Kwargs:
        ...
        # Configuration for logging is set as command-line args rather than in the bewcore.conf file.
        # This means that starting a bewcored using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        self.cli = TestNodeCLI(bewcore_cli, self.datadir_path)
    ...
    def __del__(self):
        # Ensure that we don't leave any bewcored processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time bewcored is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by bewcored, and
        ...
        self.running = True
        self.log.debug("bewcored started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the bewcored process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'bewcored exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; bewcored is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to bewcored after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; bewcored is still starting
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into bewcored")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to bewcored
        expected_msg: regex that stderr should match when bewcored fails

        Will throw if bewcored starts without an error.
        Will throw if an expected_msg is provided and it does not match bewcored's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'bewcored exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'bewcored should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to bewcore-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.bewcorecli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with bewcore-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run bewcore-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running bewcore-cli {}".format(p_args[2:]))
